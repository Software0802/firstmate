#!/usr/bin/env bash
# Pre-register Devin CLI's workspace trust for the isolated task worktree a
# ship/scout spawn is about to launch a devin crewmate into, so the worker
# reaches its brief instead of parking on the folder-trust dialog.
#
...
#
# WHY THIS EXISTS. devin 3000.10.31 gates a directory it has never seen behind
# "Do you trust the authors of this directory?" and NO hook fires while that
# dialog is on screen, so an unanswered dialog is a fully parked worker.
# Answering appends the directory to the `trusted_paths` array of
# <data>/devin/cli/trusted_workspaces.json, and devin honours an entry written
# there ahead of launch: verified live, a pre-registered directory ran its turn
# straight away while an unregistered sibling refused with
# "Refusing to run in an untrusted workspace" (docs/verification/devin.md).
#
...
# marker before the spawn reports success.
#
# THE SCOPE TEST IS THE SAFETY PROPERTY and mirrors bin/fm-agy-trust.sh and
# bin/fm-claude-trust.sh: <worktree> must be a LINKED git worktree - its own git
# dir, sharing <project>'s common dir - whose top level is exactly the resolved
...
#
# A trusted directory covers its subdirectories (verified), so registering the
# worktree root is enough for every path a worker touches inside it.
set -u
unset CDPATH \
...
# devin resolves its data root through XDG (verified: XDG_DATA_HOME moved the
# credentials path), so honour the same variable rather than hardcoding
# ~/.local/share, or a captain running an XDG-relocated devin would have trust
# written to a store the worker never reads.
DATA_HOME=${XDG_DATA_HOME:-$HOME_REAL/.local/share}
case "$DATA_HOME" in
  /*) ;;
  *) refuse "XDG_DATA_HOME '$DATA_HOME' is not an absolute path" ;;
esac
STORE_DIR="$DATA_HOME/devin/cli"
