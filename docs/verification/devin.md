...
`fm_busy_devin_turn_contradicted` in `bin/fm-busy-lib.sh` closes that hole from the read side, and it is deliberately the narrowest thing that can: it never produces a verdict of its own, it only CONTRADICTS the record devin's own `UserPromptSubmit` left open, and the verdict it produces is `unknown devin-turn-contradicted`, never `idle`, because the approved redesign forbids rendered text from proving a worker settled.
It reaches no other record devin trusts: the launch-brief turn `bin/fm-spawn.sh` seeds as `busy`/`fm-spawn`, and an `fm-recovery` reset after relaunch, are written before devin has been handed a prompt, so devin is still rendering its idle composer and corroborating them would contradict a healthy worker whose brief is about to run.
