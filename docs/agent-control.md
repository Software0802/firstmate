...
| `exit` | Stop the agent, preserving the endpoint, the worktree, and every uncommitted change. | The backend's recovery-grade classifier reports the agent gone. Already-stopped is idempotent success. When it interrupts a busy task first, it closes the busy record on the same terms the interrupt verb does, so even an exit that then fails leaves no cancelled turn recorded busy. |
