...
Devin is anchored to the exact `devin` identity on the same rule, which covers both halves of its two-process model because its front end and its `devin acp` agent child share that process name.
