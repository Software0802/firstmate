...
devin is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because it has no primary supervision protocol despite exposing a real hook surface; [`docs/verification/devin.md`](verification/devin.md) owns that evidence, including the spawn-time worktree trust pre-registration through `bin/fm-devin-trust.sh`, the firstmate-owned per-task `--config` that carries its busy hooks and turns its default commit attribution off, and its lack of any reasoning-effort flag.
...
FM_COMPOSER_GHOST_LUMA_MAX=128   # fleet-wide: highest perceived luminance (0.299R+0.587G+0.114B, 0-255) at which a foreground still counts as de-emphasised ghost/placeholder text and is stripped; inclusive. Both foreground spellings are tested, because a harness picks the spelling from the terminal: TRUECOLOR (38;2) and 256-colour (38;5) entries 16-255 through the standard xterm palette, while theme colours 0-15 are never tested. Dim/faint (SGR 2) is stripped regardless. Assumes a dark terminal theme (bin/fm-composer-lib.sh's fm_composer_strip_ghost, used by styled tmux, herdr, and Zellij reads)
