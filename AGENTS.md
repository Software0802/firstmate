...
state/               runtime records and signals; gitignored
  ...
  <id>.devin-config.json  firstmate-owned per-task devin config carrying the busy-state and turn-end hooks, reached through --config so nothing is written into the project's own .devin/; removed by teardown
  <id>.devin-session  devin session id written by that config's SessionStart hook, which is also the spawn's readiness proof; removed by teardown
