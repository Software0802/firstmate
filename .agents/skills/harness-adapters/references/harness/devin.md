...
| Busy state | Semantic `devin-hook`: `UserPromptSubmit` opens a turn, `Stop` and `SessionEnd` close it. `Stop` does NOT fire on a manual interrupt - the claude gap, not the gemini one - so `fm-send --key Escape` closes the record with `idle`/`fm-interrupt`, but only after it has delivered BOTH presses devin needs; one press alone arms the second and the turn carries on. |
...
| Composer | Bordered row whose prompt glyph is `❭` (U+276D), with the idle placeholder `Ask Devin to build features, fix bugs, or work on your code` and the mid-turn placeholder `Guide Devin while it works`. Both placeholders are drawn as muted truecolor, so a styled capture strips them and the shared classifier reads the idle composer as `empty` - which is what lets the control plane type `/exit` at all. |
