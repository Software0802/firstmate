...
A pane whose brief cannot be confirmed to run in the worktree fails the spawn, records the failure in the task status, and closes the endpoint the spawn launched; a relaunch adopts the task's existing endpoint instead, so that one is left open for a retry (`../../../../../docs/agent-control.md`).
